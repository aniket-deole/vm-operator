...
Introduce a namespace-scoped `Tag` resource in the `vsphere.policy.vmware.com/v1alpha1` API that records which label key/value pairs currently participate in affinity in a namespace and which VMs own that participation. A new `vmconfig` reconciler on the VM path — invoked from **two** call sites in `doReconfigure` that straddle the vSphere policy reconciler — creates those resources, maintains its own VM's owner reference, and emits the vCenter `TagSpec` add/remove diff for **every** participating label the VM carries — including labels the VM does not itself reference. A new `Tag` controller owns `Tag` status, its finalizer, and its delete-at-zero-owners decision. Fan-out to the VMs a `Tag` change affects is a **watch**: the VM controller gains a feature-gated `Watches(&Tag{}, …)` whose mapper resolves a `Tag` to the VMs carrying its label through a field index. All of it is gated on a new `TaggingAPI` feature flag, which also un-gates mutability of `spec.affinity`.
...
| Controllers are thin; business logic in `pkg/` | OK | `controllers/vspherepolicy/tag/` holds the reconcile loop, finalizer, patch helper, and index registration only; the VM controller's change is a `Watches` line whose mapper and predicate live in `pkg/util/kube/affinitytag.go`; the label/affinity/tag-set computation lives in `pkg/providers/vsphere/virtualmachine/affinitytag.go`, `pkg/util/kube/affinitytag.go`, and `pkg/vmconfig/affinitytag/`. |
...
pkg/providers/vsphere/virtualmachine/
  affinitytag.go                                # shared helpers + the ExtraConfig key, beside affinity.go
  affinitytag_test.go
...
With `CRDCleanupEnabled=true`, turning the flag off deletes the CRD and with it every `Tag` resource — and those hold the `vmoperator.vmware.com/tag` finalizer, whose owning controller is no longer running, so the deletion would wedge until the finalizers are cleared by hand. This is not a new hazard introduced here: `PolicyEvaluation` carries `vmoperator.vmware.com/policy-evaluation-finalizer` and is gated the same way behind `features.VSpherePolicies`. Worth knowing before anyone enables `CRDCleanupEnabled` on a cluster with live `Tag` resources; out of scope to fix for this feature.
...
### 1. Shared helpers — `pkg/providers/vsphere/virtualmachine/affinitytag.go`
...
- `ExtraConfigAffinityTagsKey` — the ExtraConfig key the tag record uses (see step 5 and "Platform-resource cleanup"; the placement is deliberate, see the import-direction note below).
...
`pkg/vmconfig/affinitytag` therefore imports `pkg/providers/vsphere/virtualmachine`. That direction is cycle-free and already idiomatic — `pkg/vmconfig/cdrom` does exactly this today, and `session` → provider `virtualmachine` is one-way (the provider package does **not** import `session`), so `session` calling both is fine.

**The rule it imposes:** no file in `pkg/providers/vsphere/virtualmachine` may import `pkg/vmconfig/affinitytag`, or the graph closes. `cleanup.go` is precisely the file that would want to, for the ExtraConfig key (see "Platform-resource cleanup"), which is why `ExtraConfigAffinityTagsKey` is declared in this package rather than in `pkg/vmconfig/affinitytag`: `cleanup.go` then reads it from its own package with no import at all, and the constraint is enforced structurally rather than by comment. This mirrors mechanism B, where the key lives with its owner (`pkg/vmconfig/policy`); the owner is simply on the other side here. (`pkg/providers/vsphere/constants`, which already holds `vmservice.namespacedName`, is the alternative home if this file ever gets crowded.)

The four field indexes are unaffected and stay in `pkg/util/kube/affinitytag.go` per the `pvc.go` / `vmsnapshot.go` convention — `pkg/util/kube` can import `external/vsphere-policy` for the `Tag` extractors with no new cycle.
...
if pkgcfg.FromContext(ctx).Features.TaggingAPI {
    var err error
    if ownedTags, err = reconcileAffinityTagCRs(
...
if pkgcfg.FromContext(ctx).Features.TaggingAPI {
    if err := reconcileAffinityTagSpecs(
...
4. **Compute the desired vCenter tag set** — the union of two sources:
   ...
   - For each distinct label key on the VM, `List` `Tag`s by the `spec.label` field index and keep the non-terminating ones whose `spec.value` also matches (see "Field indexes and query patterns" below). Equivalently: the non-terminating `Tag`s in the namespace whose `spec.label`/`spec.value` matches one of **this VM's own labels**.
   ...
   **The union is required, not an optimization.** Step 2's `Create` goes to the API server, but this `List` reads the informer cache — the provider's client is `mgr.GetClient()` (`pkg/manager/init/init_providers.go`), and `pkg/manager/manager.go` disables caching only for `ConfigMap`, `Secret` and `Deployment` — so the cache will not have observed that create within the same reconcile. Without the union, a VM that establishes a relationship would not carry its own tag until some later reconcile: spec G1 / US1.1 would converge only via the `Tag` watch's fan-out, which is a silent, timing-dependent second pass rather than the specified behavior. Cross-VM participation (label-only VMs, `Tag`s owned by others) legitimately reads through the cache and legitimately relies on the fan-out; only the self-referential case needs the union.
   ...
   The membership condition deliberately does **not** consult the VM's own `spec.affinity`: tag carriage follows labels plus `Tag` existence, ownership follows labels plus affinity (spec "Ownership vs. tag carriage"). This is the step that tags label-only VMs (spec G2), that keeps a VM tagged after it stops referencing a label it still carries (spec US3 scenario 2), and that makes participation namespace-wide rather than self-referential. Note that the union does not weaken this: the owned set is a subset of the label-matched set by construction (step 1 intersects with the VM's labels), so unioning it in can only recover a cache miss, never add a pair the VM does not carry.
...
5. **Emit the diff against this feature's own ExtraConfig record** (D12). The attached-tag list the update flow fetches is a list of tag **URNs** and is not on `moVM` (it is in the context, via `pkgctx.GetVMTags`), so it cannot be matched against name+category tags at all — see `research.md`. This feature therefore mirrors mechanism B's workflow with its own key:
   ...
   ```go
   // pkg/providers/vsphere/virtualmachine/affinitytag.go
   //
   // ExtraConfigAffinityTagsKey is the ExtraConfig key that contains the
   // "<key>:<value>" affinity tags this feature has applied, so it can later
   // remove only the tags it applied itself.
   const ExtraConfigAffinityTagsKey = "vmservice.affinity.tags"
   ```

   It is declared in the provider package, not here, so that `cleanup.go` can read it without importing `pkg/vmconfig/affinitytag` — see "Why this package, and the one rule it imposes".
   ...
   - `haveBeen` — the comma-split value of `ExtraConfigAffinityTagsKey` from `moVM.Config.ExtraConfig`.
...
3. If `len(ownerReferences) == 0`, `Delete` the `Tag` and return. The finalizer means the `Delete` only sets `deletionTimestamp`, which is itself the watch event that wakes every VM carrying the label. This is the case ordinary garbage collection does not cover (`research.md`), and it is what satisfies spec US3 scenarios 1 and 3.
4. Otherwise set `status.observedGeneration` and `Ready` per `model.md`.

`ReconcileDelete`: remove the finalizer (spec G5, D10). Nothing else — by the time this runs the VMs have already been enqueued by the `deletionTimestamp` update, and the subsequent disappearance of the object produces a second watch event that catches any VM whose reconcile raced ahead of that update.

**Why the finalizer is still needed** even though the controller no longer has fan-out work to do on the delete path: it guarantees a terminating window between "this `Tag` is going away" and "this `Tag` is gone". The VM path's desired-set computation excludes terminating `Tag`s, so a VM woken during that window converges to the same answer as one woken after the object disappears — and `ReconcileTagCRs`' deletion-race rule (spec G12: a `Tag` found terminating is requeued, never adopted) needs an observable terminating state to be well-defined at all.
...
**The mapper** (`pkg/util/kube/affinitytag.go`, following the `EncryptionClassToVirtualMachineMapper` shape in `pkg/util/vmopv1/vm.go`) resolves a `Tag` to the VMs that carry its label with a single indexed `List`:

```go
client.InNamespace(tag.Namespace),
client.MatchingFields{kubeutil.VMLabelKeyValueIndexKey: tag.Spec.Label + ":" + tag.Spec.Value}
```
...
**The predicate** exists because the `Tag` controller writes to `Tag`s on its own reconciles — status, the finalizer, the label mirror — and each VM path reconcile patches owner references. Without a filter every one of those writes would wake every VM carrying the label. `spec` is immutable by admission (`model.md` V-rules) and the mapper's key is derived from `spec` alone, so the set of interesting transitions is small:
...
| Update | only when `deletionTimestamp` goes from nil to set | The one update that changes the answer. Status, finalizer, label-mirror and owner-reference writes are filtered out. |
| Delete | yes | Backstop for a VM that reconciled during the terminating window but before its cache saw the `deletionTimestamp`. The mapper reads `spec` off the delivered object, which is still populated on a delete event. |
...
`AddToManager` on the `Tag` controller: `For(&vspherepolv1.Tag{})`, `WithLogConstructor(pkglog.ControllerLogConstructor(...))`, registration of all four field indexes (next section), registered from `controllers/vspherepolicy/controllers.go` behind the flag. No `Watches` on `VirtualMachine` is needed — each VM's own reconcile is what maintains ownership, and that reconcile is triggered by the VM controller's existing watches.
...
Following the repository's convention for indexes (`pkg/util/kube/pvc.go`'s `VMSpecVolumesPVCsIndexKey` + `VMSpecVolumesPVCsIndexerFunc`, `pkg/util/kube/vmsnapshot.go`'s `VMSnapshotVMNameFieldIndex`), the index key constants and their extractor functions live in `pkg/util/kube/affinitytag.go`, exposed together as one `RegisterAffinityTagIndexes(ctx, mgr.GetFieldIndexer()) error` helper:
...
**Registered exactly once**, from `AddToManager` on the `Tag` controller, against the shared manager cache — every client built from that cache then benefits, including the VM controller's mapper. Registering an index twice for the same object and field returns an error from the field indexer, so this must not be duplicated in the VM controller's `AddToManager`; the VM controller's mapper depends on the `Tag` controller having registered it, which holds because both are gated on `TaggingAPI` and both are wired from `controllers/controllers.go` before the manager starts. `IndexField` calls are order-independent, so it does not matter which of the two `AddToManager`s runs first.
...
| Which `Tag`s match any of this VM's labels? (step 4 of the VM path) | one `List` per **distinct label key** on the VM using the `spec.label` index, then an in-memory value comparison — **not** one list of the whole namespace — **unioned** with the `Tag`s `ReconcileTagCRs` just ensured, which the cache cannot yet return in this pass | indexed, bounded by the VM's label count |
| Which VMs carry this `Tag`'s label? (the `Tag` watch's mapper, spec US2) | `List` with `client.InNamespace(tag.Namespace)` + `client.MatchingFields{"metadata.labels.keyValue": tag.Spec.Label + ":" + tag.Spec.Value}` | indexed, returns matches only |

Every repeated query is indexed or a point read; none scans the namespace. In particular the mapper's `List` returns *only* the VMs that carry the label, so its cost scales with the size of the affinity relationship rather than with the number of VMs in the namespace — which is the property spec US2 needs, since that query runs on every `Tag` create, terminating transition, and delete.
...
### 8. Every emission point is gated: flag off = today, flag on = the `Tag`-driven path
...
Neither `CreateConfigSpec` nor `CreateConfigSpecForPlacement` has a `ctrlclient.Client` — both are pure functions — so the lookup lives in a helper called by the provider methods, which hold `vs.k8sClient`. It goes in `pkg/providers/vsphere/virtualmachine/affinitytag.go` beside the other shared helpers (D13):
...
func AppendExistingTagSpecs(
    ...
    recordApplied bool) error
```

For each of the VM's own labels (after `kubeutil.RemoveVMOperatorLabels`), `Get` the `Tag` by its derived name; if it exists and is not terminating, append `TagSpec{Operation: add, Id.NameId{Tag: "<key>:<value>", Category: vm.Namespace}}`. Nothing else:
...
### 9. Platform-resource cleanup — `cleanup.go`
...
```mermaid
flowchart TD
    subgraph VMPath1["doReconfigure #1 — affinitytag.ReconcileTagCRs"]
    ...
    subgraph VMPath2["doReconfigure #3 — affinitytag.ReconcileTagSpecs"]
        D[List non-terminating Tags in ns<br/>keep those matching vm.Labels<br/>#40;NOT vm.spec.affinity#41;] --> D1[UNION with the owned Tags<br/>from #1 — cache has not yet<br/>observed this pass's creates<br/>= desired vCenter tag set]
        D1 --> E[Diff vs ExtraConfig record<br/>vmservice.affinity.tags<br/>#40;NOT the attached URN list#41;]
    ...
    subgraph TagPath["Tag controller — controllers/vspherepolicy/tag"]
        ...
        H -- yes --> I[Delete the Tag — finalizer turns this<br/>into deletionTimestamp<br/>GC does not cover this case]
        H -- no --> J[status.observedGeneration + Ready]
        I --> I2[ReconcileDelete — release the finalizer only.<br/>No VM enqueue happens here]
    end

    subgraph WatchPath["Fan-out — VM controller's Watches#40;#38;Tag#123;#125;#41;"]
        W1[Tag event] --> W2{predicate:<br/>create, delete, or<br/>deletionTimestamp set?}
        W2 -- no --> W3[dropped — status / finalizer /<br/>ownerRef churn does not fan out]
    ...
    G -.-> W1
    I -.->|deletionTimestamp set| W1
    I2 -.->|object removed| W1
...
- `pkg/providers/vsphere/virtualmachine/cleanup_test.go` — `removeTagAssociations` emits a `NameId` removal per entry recorded in `vmservice.affinity.tags` when `TaggingAPI` is on, leaves mechanism B's UUID removals intact when both flags are on, and emits nothing for this feature when the flag is off.

### Integration (`testlabels.EnvTest`, `testlabels.VCSim`)

- `controllers/vspherepolicy/tag/tag_controller_test.go` (envtest) — real garbage collection: deleting the last owning VM removes the `Tag`; deleting one of two owners does not; concurrent owner-reference writes from two VMs both survive (the optimistic-lock guarantee); all four field indexes resolve as specified, including the negative cases for `metadata.labels.keyValue` (same key with a different value, and VMs in another namespace); a `Tag` write that only touches status or owner references bumps no VM `resourceVersion` (`Consistently`); the CRD's printer columns and status subresource behave as generated. The generated CRD needs no test-suite wiring — `test/builder/test_suite.go` already loads the whole `config/crd/external-crds` directory into envtest.
...
- `pkg/providers/vsphere/session/session_vm_update_test.go` (vcsim) — the emitted `TagSpec`s reach the reconfigure call, and an attached-tag diff round-trips. This suite also pins the three-way ordering inside `doReconfigure` — `ReconcileTagCRs` → `reconcileVSpherePolicies` → `ReconcileTagSpecs` — which is load-bearing rather than incidental (see "Ordering inside `doReconfigure`"), so assert it directly: with both `TaggingAPI` and `VSpherePolicies` on, the resulting `configSpec.TagSpecs` must contain mechanism B's UUID entries unmodified alongside this feature's `NameId` entries.
